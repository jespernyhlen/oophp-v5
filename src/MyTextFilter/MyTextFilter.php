...
class MyTextFilter
{
    ...
    {
        $filteredText = $text;
        if (strlen($text) > 0 && count($filters) > 0) {
            foreach ($filters as $filter) {
                switch ($filter) {
                    case 'bbcode':
                        $filteredText = $this->bbcode2html($filteredText);
                        break;
                    case 'markdown':
                        $filteredText = $this->markdown($filteredText);
                        break;
                    case 'link':
                        $filteredText = $this->makeClickable($filteredText);
                        break;
                    case 'nl2br':
                        $filteredText = $this->nl2br($filteredText);
                        break;
                }
            }
        }
